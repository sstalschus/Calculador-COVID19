...
//In�cio da declara��o para colis�o
...
int main() {
	...
	//Colocar m�sica para tocar
	play_sample(musicaf, 100, 128,1000, 1);

	//preencher o vetor de menus sexo e idade de sele��o com zero
	...
	while (!(sai || key[KEY_ESC] || sairr==1))
	{
		...
		//menu - come�ar
		if(colidir (mouse_x , mouse_y , 380 , 205 , 1 , 10 , 180 , 90)){
		menus[0]=200; 
			if(mouse_b == 1){parte = 1;}} 
				else {menus[0]=0;}
					{masked_blit(menup , buffer , menus[0], 0 , 380 , 200 , 200 , 100 );}
		//menu - sobre
		if(colidir (mouse_x , mouse_y , 380 , 305 , 10 , 10 , 180 , 90)){
		menus[1]=200; 
			if(mouse_b==1){parte=2;}} 
				else {menus[1]=0; }
					{masked_blit(menup , buffer , menus[1], 100 , 380 , 300 , 200 , 100 );}
		//menu - sair
		if(colidir (mouse_x , mouse_y , 380 , 405 , 10 , 10 , 180 , 90)){
		menus[2]=200; 
			if(mouse_b==1){sairr=mouse_b;}} 
				else {menus[2]=0; }
		...
		//bot�o de menu para retornar ao menu
		if(colidir (mouse_x , mouse_y , 450 , 0 , 10 , 10 , 80 , 40)){
		menus[4]=100; 
			if(mouse_b == 1){parte = 0; 
				for(i=4; i<26 ; i++){menus[i]=0;}}} 
					else {menus[4]=0;}
						{masked_blit(menuq , buffer , menus[4], 0 , 450 , 0 , 100 , 50 );}

		//bot�o de calcular para calcular o resultado
		if(colidir (mouse_x , mouse_y , 450 , 650 , 10 , 10 , 80 , 40)){
		menus[3]=100; 
			if(mouse_b == 1){parte = 3;}} 
				else {menus[3]=0;}
					{masked_blit(menucalcular , buffer , menus[3], 0 , 450 , 650 , 100 , 50 );}

		//menu do sexo masculino e feminino
		if(mouse_b == 1 && colidir (mouse_x , mouse_y , 125 , 222 , 10 , 10 , 110 , 40) && menus[5]==0){menus[5]=120; 
			if(menus[6]==120)menus[6]=0; if(menus[5]==120){sexodoenca[0]=1.0;}}
			masked_blit(menusexo , buffer , menus[5], 0 , 125 , 222 , 120 , 50 );
				if(menus[6]==0 && mouse_b == 1 && colidir (mouse_x , mouse_y , 255 , 222 , 10 , 10 , 110 , 40)){menus[6]=120; 
					if(menus[5]==120)menus[5]=0; 
						if(menus[6]==120){sexodoenca[0]=0.0;}}
						masked_blit(menusexo , buffer , menus[6], 50 , 255 , 222 , 120 , 50 );

		//menu do sim e n�o - diabetes
		if(menus[8]==0 && mouse_b == 1 && colidir (mouse_x , mouse_y , 650 , 222 , 10 , 10 , 90 , 40)){menus[8]=50; 
			if(menus[8]==50){menus[23]=0; sexodoenca[1]=1.0;} }
				masked_blit(simenao , buffer , 0 , menus[8] , 650 , 222 , 100 , 50 );
					if(menus[23]==0 && mouse_b == 1 && colidir (mouse_x , mouse_y , 765 , 222 , 10 , 10 , 90 , 40)){menus[23]=50; 
						if(menus[23]==50){menus[8]=0; sexodoenca[1]=0.0;}}
							masked_blit(simenao , buffer , 100, menus[23] , 765 , 222 , 100 , 50 );

		//menu do sim e n�o - hipertens�o
		if(menus[9]==0 && mouse_b == 1 && colidir (mouse_x , mouse_y , 650 , 325 , 10 , 10 , 90 , 40)){menus[9]=50;
			if(menus[9]==50){menus[24]=0; sexodoenca[2]=1.0;}}
				masked_blit(simenao , buffer ,0 , menus[9] , 650 , 325 , 100 , 50 );
					if(menus[24]==0 && mouse_b == 1 && colidir (mouse_x , mouse_y , 765 , 325 , 10 , 10 , 90 , 40)){menus[24]=50;
						if(menus[24]==50){menus[9]=0; sexodoenca[2]=0.0;}}
							masked_blit(simenao , buffer , 100 , menus[24] , 765 , 325 , 100 , 50 );

		//menu do sim e n�o - obesidade
		if(menus[10]==0 && mouse_b == 1 && colidir (mouse_x , mouse_y , 650 , 428 , 10 , 10 , 90 , 40)){menus[10]=50;
			if(menus[10]==50){menus[25]=0; sexodoenca[3]=1.0;} }
				masked_blit(simenao , buffer , 0 ,  menus[10] , 650 , 428 , 100 , 50 );
					if(menus[25]==0 && mouse_b == 1 && colidir (mouse_x , mouse_y , 765 , 428 , 10 , 10 , 90 , 40)){menus[25]=50;
						if(menus[25]==50){menus[10]=0; sexodoenca[3]=0.0;} }
		...
		menus[11]=100; 
			if(menus[11]=100){ idades[0]=1.0; 
				for(i=12;i<23;i++){menus[i]=0;}}}
					masked_blit(menuidade , buffer , menus[11], 0 , 90 , 330 , 100 , 50 );

		//1 � 10
		if(colidir (mouse_x , mouse_y , 205 , 330 , 10, 10 , 95, 45) && mouse_b==1 && menus[12]==0){
		menus[12]=100; 
			if(menus[12]=100){ idades[0]=5.0; 
				for(i=13;i<23;i++){menus[i]=0;} 
					for(i=11;i>10;i--){menus[i]=0;}}}
		...
		menus[13]=100; 
			if(menus[13]==100){idades[0]=15.0; 
				for(i=14;i<23;i++){menus[i]=0;} 
					for(i=12;i>10;i--){menus[i]=0;}}}
		...
		menus[14]=100; 
			if(menus[14]==100){idades[0]=25.0; 
				for(i=15;i<23;i++){menus[i]=0;} 
					for(i=13;i>10;i--){menus[i]=0;}}}
		...
		menus[15]=100; 
			if(menus[15]=100){idades[0]=35.0; 
				for(i=16;i<23;i++){menus[i]=0;} 
					for(i=14;i>10;i--){menus[i]=0;}}}
		...
		menus[16]=100; 
			if(menus[16]=100){idades[0]=45.0; 
				for(i=17;i<23;i++){menus[i]=0;} 
					for(i=15;i>10;i--){menus[i]=0;}}}
		...
		menus[17]=100; 
			if(menus[17]=100){idades[0]=60.0; 
				for(i=18;i<23;i++){menus[i]=0;} 
					for(i=16;i>10;i--){menus[i]=0;}}}
		...
		menus[18]=100; 
			if(menus[18]=100){ idades[0]=65.0; 
				for(i=19;i<23;i++){menus[i]=0;} 
					for(i=17;i>10;i--){menus[i]=0;}}}
		...
		menus[19]=100; 
			if(menus[19]=100){idades[0]=75.0; 
				for(i=20;i<23;i++){menus[i]=0;} 
					for(i=18;i>10;i--){menus[i]=0;}}}
		...
		menus[20]=100; 
			if(menus[20]=100){idades[0]=85.0; 
				for(i=21;i<23;i++){menus[i]=0;} 
					for(i=19;i>10;i--){menus[i]=0;}}}
		...
		menus[21]=100; 
			if(menus[21]=100){idades[0]=95.0;  
				for(i=22;i<23;i++){menus[i]=0;} 
					for(i=20;i>10;i--){menus[i]=0;}}}
		...
		menus[22]=100; 
			if(menus[22]=100){idades[0]=100.0; 
				for(i=21;i>10;i--){menus[i]=0;}}}
		...
		//f�rmula para calculo das chances de morrer
		if(idades[0]>60)idades[1]=1; //para definir idoso

		resultado[0]=1.0/(1.0+(pow(2.71828183 , 
								(-1.0*(-7.09
									+0.4456* sexodoenca[0] + 0.058* 
									idades[0] + 0.9866*sexodoenca[1]+0.9176*sexodoenca[2]
